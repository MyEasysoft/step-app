...
export class TransactionPanelComponent extends Component {
 ...
 getRoomId (listingPaidFor,listingSigToGet,isProvider) {
    ...
    keys.forEach(key => {
      ...
      try{
          if(listingPaidFor[key].sig === listingSigToGet){
           ...
           if(isProvider){
            room = listingPaidFor[key].videoCallData.hostRoomUrl;
           }else{
            room = listingPaidFor[key].videoCallData.roomUrl;
           }
  ...
  render() {
    ...
    const isPurchased = stateData?.processState === "accepted";
    ...
    const listingPaidFor = currentUser?.attributes?.profile?.privateData?.listingPaidFor;

    let roomUrl ="";
    if(listingPaidFor != undefined){
      console.log("uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu");
      const listingSig = provider?.id?.uuid + customer?.id?.uuid + listing?.id?.uuid;
      roomUrl = this.getRoomId(listingPaidFor,listingSig,isProvider);
          ...
          <div className={css.txInfo}>
            ...
            {showSendMessageForm ? (
              <SendMessageForm
                ...
                onGetVideoRoomUrl={onGetVideoRoomUrl}
                roomUrl={roomUrl}
              />
            ) : (
