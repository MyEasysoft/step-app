...
export const FETCH_ROOM_URL_REQUEST = 'app/TransactionPage/FETCH_ROOM_URL_REQUEST';
export const FETCH_ROOM_URL_SUCCESS = 'app/TransactionPage/FETCH_ROOM_URL_SUCCESS';
export const FETCH_ROOM_URL_ERROR = 'app/TransactionPage/FETCH_ROOM_URL_ERROR';
...
const initialState = {
  ...
  fetchRoomUrlInProgress: false,
  fetchRoomUrlSuccess: false,
  fetchRoomUrlError: null,
  roomUrl: null,
...
export default function transactionPageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case TRANSITION_ERROR:
      ...
      case FETCH_ROOM_URL_REQUEST:
        return {
          ...state,
          fetchRoomUrlInProgress: payload,
          fetchRoomUrlSuccess:null,
          fetchRoomUrlError: null,
        };
      case FETCH_ROOM_URL_SUCCESS:
        return { ...state, fetchRoomUrlInProgress: null ,roomUrl:payload,fetchRoomUrlSuccess:true};
      case FETCH_ROOM_URL_ERROR:
        return {
          ...state,
          fetchRoomUrlInProgress: null,
          fetchRoomUrlSuccess:null,
          fetchRoomUrlError: payload,
        };
...
export const fetchRoomUrlRequest = () => ({ type: FETCH_ROOM_URL_REQUEST });
export const fetchRoomUrlSuccess = roomUrl => ({
  type: FETCH_ROOM_URL_SUCCESS,
  payload: roomUrl,
});
export const fetchRoomUrlError = error => ({
  type: FETCH_ROOM_URL_ERROR,
  error: true,
  payload: error,
});
...
export const fetchVideoRoomUrl =  ({ listingId }) => async dispatch => {
 
    dispatch(fetchRoomUrlRequest());

    const response = await fetch('/api/getvideocallurl', {
      method: 'POST',
      body: "",
      headers: {
      ...
      console.log(res + "                  ttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt");
      const vidData = res;
      dispatch(fetchRoomUrlSuccess(vidData));
      return res;
  
    }).catch(err=>{
      dispatch(fetchRoomUrlError(storableError(err)));
...
export const recPaymentListingPaidFor = data => async (dispatch, getState, sdk) => {
  ...
  const response =await fetch('/api/v1/api/current_user/update_profile', {
    ...
    var userId = new UUID(user.id.uuid);
    sdk.users.show({
      id: userId,
      include: ['privateData'],
      ...
      console.log("Calling api -------oooooooooooooooooooooooooooooooooooooooooo----------");
    });
